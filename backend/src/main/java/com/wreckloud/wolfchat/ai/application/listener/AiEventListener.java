...
public class AiEventListener {
    ...
    private boolean isDirectedLobbyMessage(LobbyMessageSentEvent event, WfLobbyMessage triggerMessage, Long botUserId) {
        ...
        if (!looksLikeLobbyFollowUpIntent(event.getContent())) {
            return false;
        }
        return hasImplicitLobbyContext(event.getSenderId(), botUserId, triggerMessage == null ? null : triggerMessage.getId());
    }
    ...
    private boolean looksLikeLobbyFollowUpIntent(String content) {
    ...
    private boolean hasImplicitLobbyContext(Long senderId, Long botUserId, Long triggerMessageId) {
        ...
        for (int i = 0; i < recentMessages.size(); i++) {
            ...
            if (botUserId.equals(current.getSenderId())) {
                if (isBotMessageDirectedToSender(current, senderId)) {
                    return true;
                }
                continue;
            }
    ...
    private boolean isBotMessageDirectedToSender(WfLobbyMessage botMessage, Long senderId) {
