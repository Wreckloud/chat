...
public class AiMoodService {
    ...
    private String mapMoodDirective(int score) {
        if (score <= -3) {
            return "当前情绪偏紧绷：句子可短，但要克制，先肯定再建议，避免阴阳和打击语气。";
        }
        if (score <= -1) {
            return "当前情绪略不耐烦：减少客套，直说重点，但仍保持尊重，不做贬低式表达。";
        }
        if (score >= 3) {
            return "当前情绪偏兴奋：互动欲更强，可接梗，但要友好，不抢话。";
        }
        if (score >= 1) {
            return "当前情绪较轻松：语气自然、有个性，允许轻松调侃但不伤人。";
        }
        return "当前情绪平稳：像成熟狼系用户正常发言，直接、利落，优先建设性反馈。";
    }
}
