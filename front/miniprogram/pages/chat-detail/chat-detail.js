...
Page({
  ...
  appendMessage(message) {
    ...
    imMessageHelper.appendMessage(this, normalizedMessage, {
      isValidMessage: (current) => {
        if (!current || Number(current.conversationId) !== Number(this.data.conversationId)) {
          return false
        }
  ...
  sendWsUploadProgress(payload) {
    ...
    ws.send({
      ...
      sendType: String(this.IM_SEND_TYPE || 'SEND').toUpperCase(),
      conversationId: Number(this.data.conversationId) || 0,
      clientMsgId,
